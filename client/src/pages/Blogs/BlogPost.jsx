    ...
    try {
      ...
      return (
        <div className="flex flex-col items-center my-3 sm:my-4">
          <div className={`px-4 sm:px-6 py-2 sm:py-3 rounded-lg font-mono text-sm sm:text-base md:text-lg
      ...
      return (
        ...
        >
          {parts.map((part, i) => {
            if (i % 2 === 1) {
              return (
                <span key={i} className={`inline-block px-2 py-1 mx-1 rounded font-mono text-sm sm:text-base
                  ${theme === 'dark' ? 'bg-gray-800' : 'bg-blue-50'}`}>
                  {part}
                </span>
              );
    ...
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-rgb(11, 15, 23)' : 'bg-white'}`}>
      <div className="container mx-auto px-4 py-3 sm:py-4">
        <button
          ...
          className={`flex items-center space-x-2 text-sm sm:text-base transition-colors
            ${theme === 'dark' 
      ...
      <article className="container mx-auto px-4 py-6 sm:py-8 max-w-4xl">
        ...
        <div className={`prose max-w-none ${
          theme === 'dark' ? 'prose-invert' : 'prose-blue'
        }`}>
          {formatContent(post.content)}
        </div>
      </article>

      <footer className={`container mx-auto px-4 py-6 sm:py-8 max-w-4xl border-t mt-8 sm:mt-12
